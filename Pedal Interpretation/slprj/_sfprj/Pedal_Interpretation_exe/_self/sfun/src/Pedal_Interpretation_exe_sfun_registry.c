...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_Pedal_Interpretation_exe_process_check_sum_call( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Pedal_Interpretation_exe_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_Pedal_Interpretation_exe_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Pedal_Interpretation_exe_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_Pedal_Interpretation_exe_get_eml_resolved_functions_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "Pedal_Interpretation_exe") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Pedal_Interpretation_exe_get_eml_resolved_functions_info(nlhs,plhs,
      2,newRhs);
  }
...
extern unsigned int sf_Pedal_Interpretation_exe_method_dispatcher(SimStruct *S,
...
{
  if (!strcmp(machineName,"Pedal_Interpretation_exe")) {
    return(sf_Pedal_Interpretation_exe_method_dispatcher(simstructPtr,
...
extern void Pedal_Interpretation_exe_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    Pedal_Interpretation_exe_terminator();
    sf_debug_terminate();
  }
...
extern void Pedal_Interpretation_exe_initializer(void);
extern void Pedal_Interpretation_exe_register_exported_symbols(SimStruct* S);
extern void Pedal_Interpretation_exe_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Pedal_Interpretation_exe_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      Pedal_Interpretation_exe_debug_initialize();
    }

    Pedal_Interpretation_exe_initializer();
  }
