...
function [time, vr_max_1] = TestdataGen(source, sut,duration_tick) % SUTmodelName )
% TDG transformation 
...
%clear;
clc;
% SOURCE = 'Quat_test_jza2';
% SUTName = 'SUT';
% SOURCE = 'SpeedControlTest_v312_ASE07';
...
fix_pos([block sprintf('/Test Ctrl\nInfluence')],[block '/Bus Creator 1'],70,200*length(ReqNr));
...
%$$$$
% feedback the sequence of test case duration
% setting the transition time according to this
[time vr_max_1] = TestdataReqG([TARGET '/Validation Functions'],'Test Info',TARGET,duration_tick);
vr_max = max(vr_max_1);
exe_time = sum(time * vr_max);
TestcontrolGen_v(TARGET, time);
...
% simuluation parameter setting
solver = get_param(SOURCE, 'SolverType');
set_param(TARGET, 'SolverType',solver);
fixstep = get_param(SOURCE,'FixedStep');
set_param(TARGET,'FixedStep', fixstep);
exe_time_r = str2num(fixstep) * exe_time;
set_param(TARGET,'StopTime',  num2str(exe_time_r));
...
%==========get the value for state flow
%
% generate the coverage report of output signal

% v = [2 60];
% ref = {[2 8 10]};
% for i= 1:length(OutputName)
%     report_coverage(TARGET, SUTName, OutputName(i),ref{i},v);
% end
GenLogdata(TARGET); % Set signal as log data on the precondition level
GenLogdata_tc(TARGET); % Set signal as log data on the requirement level
close_system(TARGET, 1);
disp(sprintf('Test Structure "%s" for "%s" built.', TARGET, SOURCE));
disp(['Test execution time : ' num2str(exe_time_r) ' seconds through ' num2str(vr_max) ' time iterations']);
