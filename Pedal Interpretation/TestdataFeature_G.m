function [vr_max] = TestdataFeature_G(VFsName,TestDataName,tar,duration_tick)
...
Inter_sig = '';
Inter_sigN = {};
flag_inter = 0; % 1 represents intermediate signal out of SUT
if strcmp(OutSigName(1:6),'Output') == 1 % judge the first signal is not an Input
        l1=length('Outputs.')+1;
...
if strcmp(OutSigName(1:6),'Interm') == 1
    flag_inter = 1;
    l1=length('Intermediate.')+1;
    l2=length(OutSigName);
                    
    Inter_sig = strcat(Inter_sig,',',OutSigName(l1:l2));
    Bus_Sel_SigN1 = OutSigName_N{1};
    Bus_Sel_SigN1 = Bus_Sel_SigN1(2:(length(Bus_Sel_SigN1)-1));

    Inter_sigN = [Inter_sigN Bus_Sel_SigN1];
        
end
if isempty(h) == 0 % more than one Signal 
    for k=1:length(h)
        l3 = h(k)+1;
        if strcmp(OutSigName(l3:(l3+5)),'Output') == 1 % judges signal is not 'Inputs.' type
            if k == length(h) %the last outputs signal
                l1 = h(k)+1;
                l2 = length(OutSigName);
                Sig_1 = OutSigName(l1:l2);
                Sig_1_n = findstr(Sig_1,'.');
                Sig_1_l = length(Sig_1);
                SIGNAME = Sig_1((Sig_1_n(length(Sig_1_n))+1):Sig_1_l);
                
                Bus_Sel_Sig = strcat(Bus_Sel_Sig,',',SIGNAME);
        ...
        if strcmp(OutSigName(l3:(l3+5)),'Interm') == 1
            flag_inter = 1;
            if k == length(h)
                l1 = h(k)+1;
                l2 = length(OutSigName);
                ...
                Inter_sig = strcat(Inter_sig,',',SIGNAME);
                Bus_Sel_SigN1 = OutSigName_N{k+1};
                Bus_Sel_SigN1 = Bus_Sel_SigN1(2:(length(Bus_Sel_SigN1)-1));
...
if Bus_Sel_Sig_Nr > 0
    ...
    add_block('Simulink/Discrete/Memory',[TestDataName '/Memory ' Bus_Sel_SigN{k}]);
    fix_pos([TestDataName '/Bus Selector'],[TestDataName '/Memory ' Bus_Sel_SigN{k}],k*120,k*80);
    add_line(TestDataName,['Bus Selector/' num2str(k)],['Memory ' Bus_Sel_SigN{k} '/' num2str(1)],'autorouting','on');
    end
end
    ...
    if is_ref_l == 1 && is_sig_l == 1 % detect the Rel
     %if is_act_l == 1 && is_ref_l == 1 && is_sig_l == 1 % detect the Rel   
        ...
        if strcmp(Input1_block_name,sprintf('Bus\nSelector')) == 0 %between the bus selector and the block exists another blocks
            ...
            if strcmp(feature, 'Logic') == 1
                g1 = get_param(Input1_block, 'Linehandles');
                
                n = get_param(g1.Inport, 'SrcBlockHandle');
                
                n1 = get_param(n{1},'Linehandles');
                InputName = get_param(n1.Inport, 'Name');
                n3 = get_param(n1.Inport, 'SrcBlockHandle');
                n4 = get_param(n3,'Name');
                if strcmp(n4,sprintf('Bus\nSelector')) == 0
                    
                    Tp3 = get_param(n{1}, 'Linehandles');
                    n = get_param(Tp3.Inport(1), 'SrcBlockHandle');
                    n1 = get_param(n,'Linehandles');
                    InputName = get_param(n1.Inport, 'Name');
                end
                
                
%                InputName1 = InputName{1};
                InputName1 = InputName(2:(length(InputName)-1));
                ...
                l1 = length(Inter_sigN);
                if isempty(l1) == 0
                    for j = 1:l1
                        if strcmp(InputName1,Inter_sigN(j)) == 1
                            flag = 2;
                        end
                    end
                end
                ...
                if flag == 1 % Rel connect with the input signal
                    ...
                    if strcmp(v{1},'==')== 1 || strcmp(v{1},'Range')== 1% v{1} to char, if the constraint is '<'
                        ...
                        elseif strcmp(v{1},'>=')== 1 || strcmp(v{1},'>')==1 || strcmp(v{1},'Range')== 1
                        add_block('Simulink/Ports & Subsystems/Subsystem',[TestDataName '/Generate increase ' num2str(i)]);
                        ...
                        fix_pos([TestDataName '/Out Bus'],[TestDataName '/Generate increase ' num2str(i)],200*i,-50);
                        ...
                        if strcmp(val, 'ped_min') == 1
                           val = '5';
                        end
    %                     TP1= [TestDataName '/Generate increase ' num2str(i)]
    %                     TP2 = Model_name
    %                     tp3 = val
    %                     tp4 = InputName1
                        variants_nr = GenIncrVar_Single([TestDataName '/Generate increase ' num2str(i)], Model_name, InputName1, val,duration_tick);
                        max_var = [max_var variants_nr];
                        elseif strcmp(v{1},'<')== 1 || strcmp(v{1},'<=')== 1 || strcmp(v{1},'Range')== 1
                        add_block('Simulink/Ports & Subsystems/Subsystem',[TestDataName '/Generate decrease ' num2str(i)]);
                        ...
                        variants_nr = GenDecrVar_Single([TestDataName '/Generate decrease ' num2str(i)], Model_name, InputName1, val,duration_tick);
                        max_var = [max_var variants_nr];
                    end
                elseif flag == 2 % Rel connect with intermediate signal
                    ...
                    if isempty(m) == 0
                        ...
                        add_line(TestDataName,[sor_name '/' num2str(1)],['Bound Precond' num2str(i) '/' num2str(1)],'autorouting','on');
            ...
            if length(is_constant) == 1 && length(is_signal) ==1 % Detect Constant
                ...
                InputName = get_param(f.Inport, 'Name');
                InputName = InputName(2:(length(InputName)-1));
                add_block('Simulink/Ports & Subsystems/Subsystem',[TestDataName '/Generate constant ' num2str(i)],'Position',[435 i*50+100 470 i*50+135]);
                set_param([TestDataName '/Generate constant ' num2str(i)], 'LinkStatus', 'none');
                add_block('Simulink/Sinks/Out1',[TestDataName '/' InputName]);
                fix_pos([TestDataName '/Activate'], [TestDataName '/Generate constant ' num2str(i)],-120,75*i);
                fix_pos([TestDataName '/Generate constant ' num2str(i)],[TestDataName '/' InputName],120,0);
                add_line(TestDataName,['Generate constant ' num2str(i) '/' num2str(1)],[InputName '/' num2str(1)],'autorouting','on');
                % set Value for the constant
                dstBlockhandl = get_param(f.Outport, 'DstBlockHandle');
                dstBlock = get_param(dstBlockhandl, 'Linehandles');
                const = get_param(dstBlock.Inport(2), 'SrcBlockHandle');
                ...
                %set_param([TestDataName '/Generate constant ' num2str(i)], 'Value', constVal);
                %set_param([TestDataName '/Generate constant ' num2str(i)], 'OutDataTypeMode', ODT);
                variants_nr = GenConstVar_Single_D_1([TestDataName '/Generate constant ' num2str(i)], Model_name, InputName, ODT,duration_tick);
                max_var = [max_var variants_nr];
            end
            if length(is_increase) ==1 && length(is_signal) ==1 % Detect Increase
                ...
                InputName = get_param(f.Inport, 'Name');
                InputName = InputName(2:(length(InputName)-1));
                add_block('Simulink/Ports & Subsystems/Subsystem',[TestDataName '/Generate increase ' num2str(i)],'Position',[435 i*50+50 470 i*50+85]);
                set_param([TestDataName '/Generate increase ' num2str(i)], 'LinkStatus', 'none');
                add_block('Simulink/Sinks/Out1',[TestDataName '/' InputName]);
                fix_pos([TestDataName '/Activate'], [TestDataName '/Generate increase ' num2str(i)],-120,25);
                fix_pos([TestDataName '/Generate increase ' num2str(i)],[TestDataName '/' InputName],120,0);
                add_line(TestDataName,['Generate increase ' num2str(i) '/' num2str(1)],[InputName '/' num2str(1)],'autorouting','on');
                % set Value for the Generate increase
                dstBlockhandl = get_param(f.Outport, 'DstBlockHandle');
                dstBlock = get_param(dstBlockhandl, 'Linehandles');
                const = get_param(dstBlock.Inport(2), 'SrcBlockHandle');
                constVal = get_param(const, 'Value');
               
                variants_nr = GenIncrVar_Single_D([TestDataName '/Generate increase ' num2str(i)], Model_name, InputName,duration_tick);
                max_var = [max_var variants_nr];
            end
            if length(is_decrease) ==1 && length(is_signal) ==1 % Detect decrease  
                ...
                InputName = get_param(f.Inport, 'Name');
                InputName = InputName(2:(length(InputName)-1));
                add_block('Simulink/Ports & Subsystems/Subsystem',[TestDataName '/Generate decrease ' num2str(i)],'Position',[335 i*50+200 370 i*50+235]);
                set_param([TestDataName '/Generate decrease ' num2str(i)], 'LinkStatus', 'none');
                add_block('Simulink/Sinks/Out1',[TestDataName '/' InputName]);
                fix_pos([TestDataName '/Generate decrease ' num2str(i)],[TestDataName '/' InputName],120,0);
                add_line(TestDataName,['Generate decrease ' num2str(i) '/' num2str(1)],[InputName '/' num2str(1)],'autorouting','on');
                % set Value for the limited Ramp
                dstBlockhandl = get_param(f.Outport, 'DstBlockHandle');
                dstBlock = get_param(dstBlockhandl, 'Linehandles');
                const = get_param(dstBlock.Inport(2), 'SrcBlockHandle');
                constVal = get_param(const, 'Value');
                variants_nr = GenDecrVar_Single_D([TestDataName '/Generate decrease ' num2str(i)], Model_name, InputName,duration_tick);
                max_var = [max_var variants_nr];
            end
...
if len_true>0
            ...
            for i = 1:l
                ...
                add_block('Simulink/Sources/Constant',[TestDataName '/' n1 ' random']);
...
vr_max = max(max_var); % maximum number of variants in the preconditon
